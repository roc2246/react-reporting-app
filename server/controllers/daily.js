...
export async function dataOfDay(req, res) {
  try {
    const ordersToCount = await models.queryOrdersByDate(utilities.productionDay().today);
    const count = getCount(ordersToCount, utilities.productionDay().today);
    res.json(count);
  } catch (err) {
    console.error(err);
...
export async function manageDailyTotals(data) {
  ...
  data.FBA = totalFBA;
  models.storeTotals(data, utilities.productionDay().today);
}
